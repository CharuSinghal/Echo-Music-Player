...
class Splash : AppCompatActivity() {
    ...
    var permisionsStrings = arrayOf(Manifest.permission.READ_EXTERNAL_STORAGE,

            Manifest.permission.MODIFY_AUDIO_SETTINGS , Manifest.permission.READ_PHONE_STATE , Manifest.permission.PROCESS_OUTGOING_CALLS , Manifest.permission.RECORD_AUDIO)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_splash)

        if (!hasPermissions(this@Splash , *permisionsStrings)) {
            //we have to ask for permission

            ActivityCompat.requestPermissions(this@Splash , permisionsStrings , 131)
        ...
        when (requestCode) {
            131 -> {
                if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED && grantResults[1] == PackageManager.PERMISSION_GRANTED && grantResults[2] == PackageManager.PERMISSION_GRANTED && grantResults[3] == PackageManager.PERMISSION_GRANTED && grantResults[4] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(this@Splash , "Please grant all Permissions" , Toast.LENGTH_SHORT).show()
                    this.finish()
                }
                return
            }
